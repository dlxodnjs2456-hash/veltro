(()=>{
  ...
  // Stable domain install buttons.
  // HTS is pinned to the currently verified Windows release so Android releases cannot cause a 404.
  const RELEASES_API='https://api.github.com/repos/dlxodnjs2456-hash/veltro/releases?per_page=40';
  const HTS_INSTALLER_URL='https://github.com/dlxodnjs2456-hash/veltro/releases/download/v1.0.55/VELTRO_HTS_Setup.exe';
  async function resolveLatestInstaller(kind){
    if(kind==='hts')return HTS_INSTALLER_URL;
    ...
    const rel=(Array.isArray(releases)?releases:[]).find(r=>!r?.draft&&!r?.prerelease&&isMts(r));
    if(!rel)throw new Error('release_not_found');
    const asset=(rel.assets||[]).find(a=>/\.apk$/i.test(String(a?.name||'')));
  ...
  function installButtons(){
    ...
    const mk=(id,label,kind)=>{const b=document.createElement('button');b.type='button';b.id=id;b.textContent=label;b.style.cssText='height:40px;margin:0;border:1px solid #c9d1db;background:#fff;color:#1f2937;border-radius:7px;font-weight:800;cursor:pointer';b.onclick=async()=>{const old=b.textContent;b.disabled=true;b.textContent='최신 버전 확인 중...';try{const u=await resolveLatestInstaller(kind);window.location.assign(u)}catch(e){console.error('[installer]',e);alert('설치파일을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.')}finally{b.disabled=false;b.textContent=old}};return b};
    box.append(mk('veltroHtsDownload','HTS 다운로드','hts'),mk('veltroMtsDownload','MTS 다운로드','mts'));card.appendChild(box);
    const note=document.createElement('div');note.textContent='HTS는 검증된 최신 Windows 설치파일, MTS는 최신 Android 설치파일로 연결됩니다.';note.style.cssText='margin-top:7px;text-align:center;color:#98a2b3;font-size:11px';card.appendChild(note);
  }
